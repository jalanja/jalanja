...
?>
    ...
    <?php endif; ?>
	   ...
	   <script type="text/javascript">
		  function showOrderPayinfo(id){
		     $("#addpaymentdialog_"+id).show();
		  }
		  function closeOrderPayinfo(id){
		    $("#addpaymentdialog_"+id).hide();
		  }
		  function submitOrderPayinfo(id){
			...
			if(pay_amount==''){
			   alert('<?php echo $this->_var['lang']['y_fill_in']; ?><?php echo $this->_var['lang']['y_pay_amount']; ?>');
			   return;
			}
			if(pay_time==''){
			   alert('<?php echo $this->_var['lang']['y_fill_in']; ?><?php echo $this->_var['lang']['y_pay_time']; ?>');
			   return;
			}
			if(pay_bank==''){
			   alert('<?php echo $this->_var['lang']['y_fill_in']; ?><?php echo $this->_var['lang']['y_pay_bank']; ?>');
			   return;
			}
			if(pay_account==''){
			   alert('<?php echo $this->_var['lang']['y_fill_in']; ?><?php echo $this->_var['lang']['y_pay_account']; ?>');
			   return;
			}
			if(payee_bank==''){
			   alert('<?php echo $this->_var['lang']['y_select_bank']; ?>');
			   return;
			}
			var params = 'order_sn='+id+'&pay_amount='+pay_amount+'&pay_time='+pay_time+'&pay_bank='+pay_bank+'&pay_account='+pay_account+'&payee_bank='+payee_bank+'&note='+note;
			Ajax.call('user.php?act=order_payinfo', params, orderPayinfoResponse, 'POST', 'JSON');
		  }
		  /**
            * 接收返回的信息
           */
          function orderPayinfoResponse(res)
          {
            if(res.error)
            {
               alert(res.message);
...
?>
          <tr>
            ...
            <td align="center" bgcolor="#ffffff">
			 ...
			 <?php if ($this->_var['item']['pay_id'] == 2): ?>
			  <br>
			  <a href="javascript:;" onclick="javascript:showOrderPayinfo(<?php echo $this->_var['item']['order_sn']; ?>);">
			   <?php if ($this->_var['item']['payinfo']['status'] == 0): ?>
			    <?php echo $this->_var['lang']['y_submit_payinfo']; ?>
			   <?php elseif ($this->_var['item']['payinfo']['status'] == 2): ?>
			    <?php echo $this->_var['lang']['y_view_payinfo']; ?>
			   <?php else: ?>
			    <?php echo $this->_var['lang']['y_edit_payinfo']; ?>
			   <?php endif; ?>
			  </a>
			  <div class="center_pop" id="addpaymentdialog_<?php echo $this->_var['item']['order_sn']; ?>" style="height:250px;top:180px;left:350px;padding:0 10px 10px 10px;display:none;text-align:left">
                <div class="center_pop_close"><span style="font-weight:bold;color:#ff4560"><?php echo $this->_var['lang']['y_submit_payinfo']; ?></span><a href="javascript:;" onclick="javascript:closeOrderPayinfo(<?php echo $this->_var['item']['order_sn']; ?>);"></a></div>
                <form name="form_<?php echo $this->_var['item']['order_sn']; ?>" enctype="multipart/form-data" style="margin-top:35px">
				 <table width="100%" cellspacing="0" cellpadding="0">
				  <tr>
				   <td><?php echo $this->_var['lang']['order_number']; ?>：</td>
				   <td colspan="3"><span style="font-weight:bold"><?php echo $this->_var['item']['order_sn']; ?></span></td>
				  </tr>
				  <?php if ($this->_var['item']['payinfo']['status'] == 3): ?>
				   <tr>
				    <td><?php echo $this->_var['lang']['y_verify_error']; ?>：</td>
				    <td colspan="3"><span style="color:red"><?php echo $this->_var['item']['payinfo']['operate_note']; ?></span></td>
				   </tr>
				  <?php endif; ?>
				  <tr height="35px">
				   <td><?php echo $this->_var['lang']['y_pay_amount']; ?>：</td>
				   <td><input type="text" name="pay_amount" style="width:150px;" value="<?php if ($this->_var['item']['payinfo']['pay_amount'] == ''): ?><?php echo $this->_var['item']['order_amount']; ?><?php else: ?><?php echo $this->_var['item']['payinfo']['pay_amount']; ?><?php endif; ?>"></td>
				   <td><?php echo $this->_var['lang']['y_pay_time']; ?>：</td>
				   <td><input type="text" name="pay_time" style="width:150px;" value="<?php echo $this->_var['item']['payinfo']['pay_time']; ?>"></td>
				  </tr>
				  <tr>
				   <td><?php echo $this->_var['lang']['y_pay_bank']; ?>：</td>
				   <td><input type="text" name="pay_bank" style="width:150px;" value="<?php echo $this->_var['item']['payinfo']['pay_bank']; ?>"></td>
				   <td><?php echo $this->_var['lang']['y_pay_account']; ?>：</td>
				   <td><input type="text" name="pay_account" style="width:150px;" value="<?php echo $this->_var['item']['payinfo']['pay_account']; ?>"></td>
				  </tr>
				  <tr height="35px">
				   <td><?php echo $this->_var['lang']['y_payee_bank']; ?>：</td>
				   <td clospan="3">
				    <select name="payee_bank">
                      <option value=""><?php echo $this->_var['lang']['y_select_bank']; ?></option>
					  <option value="Bank-BCA" <?php if ($this->_var['item']['payinfo']['payee_bank'] == 'Bank-BCA'): ?>selected<?php endif; ?>>Bank BCA</option>
					  <option value="Bank-Mandiri" <?php if ($this->_var['item']['payinfo']['payee_bank'] == 'Bank-Mandiri'): ?>selected<?php endif; ?>>Bank Mandiri</option>
					  <option value="Alipay" <?php if ($this->_var['item']['payinfo']['payee_bank'] == 'Alipay'): ?>selected<?php endif; ?>>Alipay</option>                    
					</select>
				   </td>
				  </tr>
				  <tr>
				   <td valign="top"><?php echo $this->_var['lang']['y_note']; ?>：</td>
				   <td colspan="3">
				    <textarea name="note" rows="4" cols="40"><?php echo $this->_var['item']['payinfo']['note']; ?></textarea>
				   </td>
				  </tr>
				  <?php if ($this->_var['item']['payinfo']['status'] != 2): ?>
				  <tr height="25px">
				  <td>&nbsp;</td>
				  <td colspan="3">
				   <input type="button" class="bnt_blue_1" onclick="javascript:submitOrderPayinfo(<?php echo $this->_var['item']['order_sn']; ?>);" value="<?php echo $this->_var['lang']['submit']; ?>">
				  </td>
				  </tr>
				  <?php endif; ?>
				 </table>
...
?>
        <tr>
          ...
          <td align="left" bgcolor="#ffffff"><?php echo $this->_var['item']['query_link']; ?></td>
        </tr>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      </table>
      <script>
          ...
          <tr>
            <td bgcolor="#ffffff">
              ...
              <?php elseif ($this->_var['goods']['goods_id'] > 0 && $this->_var['goods']['extension_code'] == 'package_buy'): ?>
                <a href="javascript:void(0)" onclick="setSuitShow(<?php echo $this->_var['goods']['goods_id']; ?>)" class="f6"><?php echo $this->_var['goods']['goods_name']; ?><span style="color:#FF0000;"><?php echo $this->_var['lang']['package_promotion']; ?></span></a>
...
?>
                ...
                <tr>
                  ...
                  <?php echo $this->_var['lang']['require_field']; ?></td>
                  <td align="right" bgcolor="#ffffff"><?php echo $this->_var['lang']['postalcode']; ?>：</td>
                  <td align="left" bgcolor="#ffffff"><input name="zipcode" type="text" class="inputBg" id="zipcode_<?php echo $this->_var['sn']; ?>" value="<?php echo htmlspecialchars($this->_var['consignee']['zipcode']); ?>" />
				  <?php echo $this->_var['lang']['require_field']; ?></td>
                </tr>
                <tr>
                  <td align="right" bgcolor="#ffffff"><?php echo $this->_var['lang']['phone']; ?>：</td>
