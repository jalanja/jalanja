    ...
    <div id="tabbody-div">
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
        ...
        <table width="90%" id="general-table" align="center">
		  ...
		  <tr>
            <td class="label"><?php echo $this->_var['lang']['lab_goods_url']; ?></td>
            <td><input type="text" name="goods_url" value="<?php echo htmlspecialchars($this->_var['goods']['goods_url']); ?>" size="30" onblur="checkGoodsUrl(this.value,'<?php echo $this->_var['goods']['goods_id']; ?>')" /><span id="goods_url_notice"></span>
          </tr>
          <tr>
            <td class="label">
...
?>
  ...
  function validate(goods_id)
  {
	  ...
	  validator.required('goods_url',goods_url_not_null);
	  ...
	  var urlcallback = function(res)
      {
       if (res.error > 0)
       {
         alert("<?php echo $this->_var['lang']['goods_url_exists']; ?>");
		 return;
       }
      }
      Ajax.call('goods.php?is_ajax=1&act=check_goods_url', "goods_url=" + goods_url + "&goods_id=" + goods_id, urlcallback, "GET", "JSON");
...
?>
  ...
  * 检查goods_url是否可用
  */
  function checkGoodsUrl(goods_url, goods_id)
  {
    if (goods_url == '')
    {
        document.getElementById('goods_sn_notice').innerHTML = "";
    ...
    var callback = function(res)
    {
      if (res.error > 0)
      {
        document.getElementById('goods_url_notice').innerHTML = res.message;
        document.getElementById('goods_url_notice').style.color = "red";
      }
      else
      {
        document.getElementById('goods_url_notice').innerHTML = "";
      }
    }
    Ajax.call('goods.php?is_ajax=1&act=check_goods_url', "goods_url=" + goods_url + "&goods_id=" + goods_id, callback, "GET", "JSON");
  }
