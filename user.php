...
elseif ($action == 'order_list')
{
	...
	foreach($orders as $k=>$v){
	   if($v['pay_id']==2){
	      $sql = "select pay_amount,pay_time,pay_bank,pay_account,payee_bank,note,status,operate_time,operate_note from ".$ecs->table('order_payinfo')." where order_id = ".$v['order_id']." limit 1";
	      $row = $db->getRow($sql);
		  $row['pay_time'] = $row['pay_time'] ? $row['pay_time'] : local_date('d/m/Y');
		  $orders[$k]['payinfo'] = $row;
	   }
	}
	   ...
	   if($order_id=$db->getOne($sql)){
	      ...
	      if($id=$db->getOne("select id from ".$ecs->table('order_payinfo')." where order_id = $order_id limit 1")){
			 ...
			 if($db->update($row,$ecs->table('order_payinfo'), $where = array('id'=>$id))){
			     $result = array('error'=>0,'message'=>$_LANG['y_edit_success']);
			 }else{
			     $result['message'] = $_LANG['y_edit_fail'];
			 }
		  }
		  //add
		  else{
			 ...
			 $row['user_id'] = $user_id;
			 $row['add_time'] = gmtime();  //date('Y-m-d G:i:s',time());
			 if($db->autoExecute($ecs->table('order_payinfo'), $row)){
			     $result = array('error'=>0,'message'=>$_LANG['y_submit_success']);
			 }else{
			     $result['message'] = $_LANG['y_submit_fail'];
			 }
		  }
	   }
	   else{
	      $result['message'] = $_LANG['y_ordersn_error'];
	   }
	}
	else{
	   $result['message'] = $_LANG['y_ordersn_no_exist'];
	}
...
/* 邀请方式_新增加 开始 by www.edait.cn */
elseif ($action == 'sendmail_act')
{
if ($affiliate['config']['separate_by'] == 0)
{
$title = ($_CFG['shop_name'].' friend send Angpao to you');
}else{
$title = ($_CFG['shop_name'].' Friend want shopping with you');
}
//$title = $_REQUEST['title']; //自行输入标题
//$title = "好友邀请注册"; //自定义内容
//$title = $_CFG['shop_name']; //调用网店名
//$content = $_REQUEST['content'];  //只输入显示附言
$url =($ecs->url());
$userid = ($user_id);
...
if ($affiliate['config']['separate_by'] == 0)
{
$content = ('HI,I am：'.$_REQUEST['name'].'<p>'.$_REQUEST['text'].'</p><p>Click to get invitation：<a href='.$url.'?u='.$userid.' target=_blank>'.$url.'?u='.$userid.'</a></p>');
}else{
$content = ('HI,I am：'.$_REQUEST['name'].'<p>'.$_REQUEST['text'].'</p><p>Click to get invitation：<a href='.$url.'?u='.$userid.' target=_blank>'.$url.'?u='.$userid.'&id='.$goodsid.'</a></p>');
}
...
{
$email = $value;
send_mail($email, $email, $title, $content, 1); //html格式内容
//send_mail($email, $email, $title, $content);  //文字格式内容
}
show_message('Email is flying to your friends now');
//echo "<script>alert('邀请邮件发送完毕！');window.top.location.href='user.php?act=affiliate'</script>"; //弹出提示
}
/* 邀请方式_新增加 结束 by www.edait.cn */
?>