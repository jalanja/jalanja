...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price, g.sales_volume, '.
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		$row['jiesheng']  = price_format($discount_arr['jiesheng']); //jiesheng
		/* 折扣节省计算 by ecmoban end */
