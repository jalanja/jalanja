...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        //限定语言项
        $lang_array = array('zh_cn', 'zh_tw', 'en_us', 'id_id');
 ...
 * @return  string
 */
function price_format($price, $change_price=true, $only_number = false)
{  
    if($price==='')
    {
       $price=0;
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'goods':
            ...
            {
			    $sql = "select goods_url from ".$GLOBALS['ecs']->table('goods')." where goods_id = $gid limit 1";
				$goods_url = $GLOBALS['db']->getOne($sql);
				if($rewrite){
				   if(empty($goods_url)){
				      $uri = 'goods-'.$gid;
				   }
...
if (!function_exists('array_combine')) {
    function array_combine($keys, $values)
    {
		...
		/* 向会员发红包 by alex 20150612 */
function send_register_bonus($user_id,$bid=0)
{
		if($bid ==0)
	    {
		   $bid = $GLOBALS['db']->getOne("select type_id from " . $GLOBALS['ecs']->table('bonus_type') . "where type_name ='New Member' order by type_id desc limit 1 " );
		}
		if(! $bid)
	    {
		  return  false;
		...
		send_mail($user_info['user_name'], $user_info['email'], 'New Member', $content, $tpl['is_html']);
}
