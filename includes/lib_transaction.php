...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'order_status'   => $GLOBALS['_LANG']['os'][$row['order_status']] . ',' . $GLOBALS['_LANG']['ps'][$row['pay_status']] . ',' . $GLOBALS['_LANG']['ss'][$row['shipping_status']],
					   'order_amount'   => price_format($row['order_amount'], false, true),
