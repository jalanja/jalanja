...
{ 
    ...
    $_SESSION['flow_consignee'] = $consignee;
	        if($consignee['country']>0){
		     $str = $GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id = ".$consignee['country']." ");
		    }
		    if($consignee['province']>0){
		     $str .= " ".$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id = ".$consignee['province']." ");
		    }
		    if($consignee['city']>0){
		     $str .= " ".$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id = ".$consignee['city']." ");
		    }
		    if($consignee['district']>0){
		     $str .= " ".$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id = ".$consignee['district']." ");
		    }
		    $consignee['address']= $str." ".$consignee['address'];
