...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','order_payinfo','act_order_payinfo');
...
/* 支付信息 */
elseif ($action=='order_payinfo'){
    $order_id = isset($_REQUEST['order_id'])? intval($_REQUEST['order_id']): 0;
	$order_info = $db->getRow("select order_sn,order_amount from ".$ecs->table('order_info')." where order_id = $order_id ");
	if(!$order_info){
        $err->show($_LANG['back_home_lnk'], './');
        exit;
	}
	$payinfo = $db->getRow("select * from ".$ecs->table('order_payinfo')." where order_id = $order_id ");
	if(!$payinfo){
	    $payinfo['order_sn'] = $order_info['order_sn'];
	    $payinfo['pay_amount'] = price_format($order_info['order_amount'], $change_price=true, $only_number = true);
	    $payinfo['pay_time'] = local_date($GLOBALS['_CFG']['date_format']);
	}
	$smarty->assign('payinfo', $payinfo);
	
    $smarty->display('user_transaction.dwt');
}

/* 操作支付信息 */
elseif($action == 'act_order_payinfo'){
    $order_sn = isset($_REQUEST['order_sn']) ? htmlspecialchars(trim($_REQUEST['order_sn'])):'';
	$row = array(
	 'pay_amount' => htmlspecialchars(trim($_REQUEST['pay_amount'])),
	 'pay_time' => htmlspecialchars(trim($_REQUEST['pay_time'])),
	 'pay_bank' => htmlspecialchars(trim($_REQUEST['pay_bank'])),
	 'pay_account' => htmlspecialchars(trim($_REQUEST['pay_account'])),
	 'payee_bank' => htmlspecialchars(trim($_REQUEST['payee_bank'])),
	 'note' => htmlspecialchars(trim($_REQUEST['note'])),
	);  
	$sql = "select order_id from ".$ecs->table('order_info')." where order_sn = '".$order_sn."' and user_id = $user_id limit 1";
    if($order_id=$db->getOne($sql)){
	      //edit
	      if($id=$db->getOne("select id from ".$ecs->table('order_payinfo')." where order_id = $order_id limit 1")){
		     $row['status'] = 1;
			 if($db->update($row,$ecs->table('order_payinfo'), $where = array('id'=>$id))){
				 show_message($_LANG['y_edit_success'], $_LANG['back_up_page'], 'user.php?act=order_list', 'info');
			 }
		  }
		  //add
		  else{
			 ...
			 $row['user_id'] = $user_id;
			 $row['add_time'] = gmtime();  
			 if($db->autoExecute($ecs->table('order_payinfo'), $row)){
				 show_message($_LANG['y_submit_success'], $_LANG['back_up_page'], 'user.php?act=order_list', 'info');
			 }
		  }
	}
}
...

{
    ...
    if(is_array($orders)){
        ...
        foreach($orders as $vo){
            ...
            $tracking = ($vo['shipping_id'] > 0) ? '<a href="user.php?act=order_tracking&order_id='.$vo['order_id'].'" class="c-btn3">'.$_LANG['label_track_packages'].'</a>':'';

            $asyList[] = array(

                'order_status' => $_LANG['order_status'].':'.$vo['order_status'],
            ...
            <tr>
                ...
                <td>'.$_LANG['order_number'].'：'.$vo['order_sn'].'<br>

                '.$_LANG['amount'].'：'.$vo['total_fee'].'<br>

                '.$_LANG['bought_time'].'：'.$vo['order_time'].'</td>
...

{
    ...
    foreach($consignee_list as $k=>$v){
	    if($v['country']>0){
		   $str = $db->getOne("select region_name from ".$ecs->table('region')." where region_id = ".$v['country']." ");
		}
		if($v['province']>0){
		   $str .= " ".$db->getOne("select region_name from ".$ecs->table('region')." where region_id = ".$v['province']." ");
		}
		if($v['city']>0){
		   $str .= " ".$db->getOne("select region_name from ".$ecs->table('region')." where region_id = ".$v['city']." ");
		}
		if($v['district']>0){
		   $str .= " ".$db->getOne("select region_name from ".$ecs->table('region')." where region_id = ".$v['district']." ");
		}
		$consignee_list[$k]['address'] = $str.' '.$v['address']; 
	}
    ...
    if(is_array($comment_list)){
        ...
        foreach($comment_list as $key=>$vo){
            ...
            <tr>
                ...
                <td><span style="float:right"><a href="user.php?act=del_cmt&id='.$vo['comment_id'].'" onclick="if (!confirm(\''.$_LANG['confirm_remove_msg'].'\')) return false;" style="color:#1CA2E1">删除</a></span>'.$_LANG['comments_rank'].':'.$vo['cmt_name'].' - '.$vo['formated_add_time'].' </td>
