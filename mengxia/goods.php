...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	{
	    $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') .
                " WHERE goods_url = '$_POST[goods_url]' AND is_delete = 0 AND goods_id <> '$_POST[goods_id]'";
        if ($db->getOne($sql) > 0)
        {
            sys_msg($_LANG['goods_url_exists'], 1, array(), false);
        }
	}
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_url, goods_name_style, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_url', '$goods_name_style', '$goods_sn', '$catgory_id', " .
...
elseif ($_REQUEST['act'] == 'check_goods_url')
{
    ...
    $goods_id = intval($_REQUEST['goods_id']);
    $goods_url = htmlspecialchars(json_str_iconv(trim($_REQUEST['goods_url'])));

    /* 检查是否重复 */
    if (!$exc->is_only('goods_url', $goods_url, $goods_id))
    {
        make_json_error($_LANG['goods_url_exists']);
    }
    make_json_result('');
}
