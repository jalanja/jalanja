...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	//删除背景图
	$banner = $db->getOne("select banner from ".$ecs->table('favourable_activity')." where act_id = $id");
	$pre = getcwd();
	chdir('../');
	@unlink($banner);
	chdir($pre);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        $id = intval($_GET['id']);
        $favourable = favourable_info($id);
		$favourable['start_time'] = date('Y-m-d H:i:s',strtotime($favourable['start_time']));
		$favourable['end_time'] = date('Y-m-d H:i:s',strtotime($favourable['end_time']));
        if (empty($favourable))
        {
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	// 主图上传
    if ($_FILES['banner']['name'] && $_FILES['banner']['size'] > 0)
    {
                /* 检查文件合法性 */
                if(!get_file_suffix($_FILES['banner']['name'], $allow_suffix))
                {
                ...
                $name .= '.' . end(explode('.', $_FILES['banner']['name']));
                $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;

                if (move_upload_file($_FILES['banner']['tmp_name'], $target))
                {
                    $banner = DATA_DIR . '/afficheimg/' . $name;
                }
				//更新主图，先删除原图
				if(intval($_POST['id'])>0){
				    $sql = "select banner from ".$ecs->table('favourable_activity')." where act_id = ".$_POST['id']." limit 1";
					$pre_banner = $db->getOne($sql);
					$pre = getcwd();
					chdir('../');
					@unlink($pre_banner);
					chdir($pre);
