...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        if ($order['user_id'] > 0)
        {
			...
			foreach($goods as $v){
			   var_dump($GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('goods')." set sales_volume = sales_volume + 1, sales_count = sales_count + ".$v['goods_number']." where goods_id = ".$v['goods_id'].""));
			}
